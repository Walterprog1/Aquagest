...
// Nota: Estas credenciales deberían estar en variables de entorno en producción
...
/**
 * Obtiene el Token de acceso para la API de Nave
 */
...
/**
 * Genera una intención de pago (Link de Pago)
 * @param {Object} orderData - Datos del pedido (id, total, cliente)
 */
                ...
                transactions: [
                    {
                        ...
                        description: `Pedido AquaGest #${orderData.id.split('-')[0]}`
                    }
                ],
                urls: {
                    callback_url: window.location.origin, // Donde vuelve el cliente tras pagar
                }
            })
        });

        const data = await response.json();
        return data.checkout_url; // URL para redirigir al cliente
    } catch (error) {
        console.error("Error creando link de pago Nave:", error);
        throw error;
    }
};
